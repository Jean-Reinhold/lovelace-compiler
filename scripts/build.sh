...
log "Building Lovelace Compiler..."
...
log "Generating parser from src/lovelace/Lovelace.jj..."
cd src/lovelace
if [ "$QUIET" -eq 1 ]; then
    javacc Lovelace.jj > /dev/null 2>&1
else
    javacc Lovelace.jj
fi
...
if [ "$QUIET" -eq 1 ]; then
    javac -d . $(find src -name "*.java") 2>&1 | grep -i error || true
else
